...
def blend_exe():
	...
	bp_start=np.argmin(np.abs(lam_exe-(lam[breakpoints[1]]-z_max-50)))

	blending_points=[bp_start,2*bp_exe-bp_start]

	curve_blend_js=blend_js(curve_exe_js,[0,bp_exe,len(curve_exe_js)],lam_exe,max(bp_exe-bp_start,1))
